...
k_folds = 6;
theta = zeros(8, k_folds);  % 5 παράμετροι + σφάλμα
Em=0;
for fold = 1:k_folds
    theta0 = zeros(8, 1);  % 5 παράμετροι (χωρίς τον όρο 1/(s+1)x)
   
    x0 = [theta0];
    
    theta_temp = zeros(7, 1);
    gamma=5*[1,1,1,1,1,1,1];
    for i = 1:6
        if i ~= fold
            y = @(t) interp1(t_data, X(:,i), t, 'linear', 'extrap');
            u = @(t) interp1(t_data, U(:,i), t, 'linear', 'extrap');
            [~, x] = ode15s(@(t,x) odefun3(t, x, u,y, gamma), t_data, x0);
            theta_temp = theta_temp + 0.2 * mean(x(:, 2:8))';
        end
    end
...
[~, idx] = min(theta(end, :));
theta_final = theta(1:7, idx);  % Οι 5 παράμετροι (χωρίς το σφάλμα)
disp(theta_final);
...
u4=sin(20*t_data).^(3)'+ones(length(t_data),1);
...
function dx=odefun2(t,x,u,theta)
    ...
    dx=theta(1)*phi1+theta(2)*phi2+theta(3)*phi3+theta(4)*phi4+theta(5)*phi5+theta(6)*phi6+theta(7)*phi7;
...
function dZ=odefun3(t,Z,u,y,gamma)
    ...
    dx=theta(1)*phi1+theta(2)*phi2+theta(3)*phi3+theta(4)*phi4+theta(5)*phi5+theta(6)*phi6+theta(7)*phi7+20*(y(t)-x);
    ...
    dtheta(6)=gamma(6)*(y(t)-x)*phi6;
    dtheta(7)=gamma(7)*(y(t)-x)*phi7;

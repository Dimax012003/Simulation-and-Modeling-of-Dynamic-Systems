[e,parameters]=create_error_graph_4(0.1);
function [e,parameters]=create_error_graph_4(Ts)
[t,x,u]=getsignals();
...
noisy_x1=x(k,1)+sigma*randn(length(k),1);
noisy_x2=x(k,2)+sigma*randn(length(k),1);
noisy_u=u(k)+sigma*randn(length(k),1);

[x1,~]=lsim(sys1,noisy_x1,t_new);
[x2,~]=lsim(sys2,noisy_x1,t_new);
[x3,~]=lsim(sys3,noisy_u,t_new);
